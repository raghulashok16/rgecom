...
import { fetchProducts, runSeeder } from '../../api/backendCalls';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetchProducts(0, 1, 'asc')
      .then((res) => setProductCount(res.data.totalElements ?? 0))
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      setSeedResult({ ok: true, msg: `Seeded: ${res.data.categoriesCreated} categories, ${res.data.suppliersCreated} suppliers, ${res.data.productsCreated} products.` });
    } catch {
      setSeedResult({ ok: false, msg: 'Seeding failed.' });
    } finally {
      setSeeding(false);
    }
  };
      ...
      <aside className={styles.sidebar}>
        ...
        {roles.includes('ADMIN') && (
          <div className={styles.seedSection}>
            {seedResult && (
              <p className={`text-xs mb-2 ${seedResult.ok ? 'text-green-400' : 'text-red-400'}`}>{seedResult.msg}</p>
            )}
            <button
              onClick={handleSeed}
              disabled={seeding || productCount > 0}
              title={productCount > 0 ? `Disabled — ${productCount} products already exist` : ''}
              className={styles.seedBtn}
            >
              {seeding ? 'Seeding...' : 'Seed Data'}
            </button>
            {productCount > 0 && (
              <p className={styles.seedCount}>{productCount} products exist</p>
            )}
          </div>
        )}
